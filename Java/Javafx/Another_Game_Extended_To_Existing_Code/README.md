...
Press "s" key would save the current state of the game, and press "L" key would load the last saved state. Load without any saved state would have no effect.
...
It also contains a value of `colours: ["red","green","blue"]` representing the score colour and the slime colour the level would contain.
...
The square cat is configured as shown below:
```json
      "squareCat": {
        "type": "squareCat",
        "image": "squareCat.png",
